...
public class DiscardPlanServiceImpl<E> extends TransactionServiceImpl implements TransPlanService<E>
{
	...
	private File getdownloadFile(DiscardPlanBo planInfo)
	{
		...
		try
		{
			...
			/*
			 * 
			 * excelIOimpl.writeLabel(sheet, 6, 13, assignPlanBo.getTransInfo().getOutDept()); excelIOimpl.writeLabel(sheet, 6,10, assignPlanBo.getTransInfo().getInDept());
	...
	public String getSumInfo(List<String> transIDList)
	{
		 ...
		 */
 		...
 		List<OperateRecord> recordList = DaoContext.getInstance().getOperateRecordDao().getByTransID(transIDList);

		for (OperateRecord record : recordList)
		{
			assetsNum++;
			originalValue = originalValue + record.getAssets().getOriginalValue();
			if (ComputeService.cptValue(record.getAssets().getPurchaseDate(), record.getAssets().getExpectYear(), record.getAssets().getOriginalValue()) > 0)
			{
				tValue = tValue + ComputeService.cptValue(record.getAssets().getPurchaseDate(), record.getAssets().getExpectYear(), record.getAssets().getOriginalValue());
			}
			if (record.getAssets().getDueDate().after(DateService.getCurrentDate()))
			{
				GDZCNum = GDZCNum + 1;
			}
			else
		...
		String str = "统计期间" + "处置业务发生" + transIDList.size() + "笔,涉及资产数量" + assetsNum + "个；涉及资产原值" + originalValue + "元，净值" + tValue + "元；其中到期资产" + GDZCNum + ",个；未到期资产" + DZYHPNum + "个";
